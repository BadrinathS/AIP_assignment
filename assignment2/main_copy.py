...
class NCut:
    ...
    def process(self, img):
        ...
        # cv2.imwrite('/data3/likith/AIP_assignment2/Results/Figures4/graph.png', W_img)
...
class CricketDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        segmentation_map = np.array(Image.open(segmentation_map_path).convert('RGB'))
...
class FCNWithSkip(nn.Module):
    ...
    def forward(self, x):
        ...
        # Upsample the feature map from the second last layer
        x_skip = self.conv_skip(x)
        x_skip = nn.functional.interpolate(x_skip, scale_factor=2, mode='bilinear', align_corners=False)
        return x_skip
    ...
    #     # Validation loop
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                # print(labels.cpu().numpy().shape, outputs.cpu().numpy().shape)
                ...
                print(outputs[0,0,:,:].cpu().numpy().max(), outputs[0,0,:,:].cpu().numpy().min())
                print(labels[0,0,:,:].cpu().numpy().max(), labels[0,0,:,:].cpu().numpy().min())
                plt.figure()
                
                plt.subplot(1,3,1)
                plt.imshow(255.0*((outputs[0,0,:,:].cpu().numpy()-outputs[0,0,:,:].cpu().numpy().min())/(outputs[0,0,:,:].cpu().numpy().max()-outputs[0,0,:,:].cpu().numpy().min())), cmap='gray')
                plt.subplot(1,3,2)
                plt.imshow(255.0*((labels[0,1,:,:].cpu().numpy()-labels[0,1,:,:].cpu().numpy().min())/(labels[0,1,:,:].cpu().numpy().max()-labels[0,1,:,:].cpu().numpy().min())), cmap='gray')
                plt.subplot(1,3,3)
                plt.imshow(np.transpose(images[0,:,:,:].cpu().numpy(), (1,2,0)))#.permute(1,2,0))
                plt.savefig('./plt.png')
                #Another method
                ...
                # labels_indices = labels.view(-1).long()  # Convert labels to indices
                # predicted_indices = predicted.view(-1).long()  # Convert predicted to indices
                # labels_one_hot = torch.nn.functional.one_hot(labels_indices, num_classes=9).unsqueeze(0).permute(0, 3, 1, 2)  # Convert labels to one-hot
                # predicted_one_hot = torch.nn.functional.one_hot(predicted_indices, num_classes=9).unsqueeze(0).permute(0, 3, 1, 2)  # Convert predicted to one-hot
                accuracy = accuracy_score(labels.cpu().numpy().reshape(images.shape[0],-1), predicted.cpu().numpy())
                # iou = jaccard_score(labels_one_hot, predicted_one_hot, average='macro')
                
                # target = torch.randint(0, 2, (10, 25, 25))
                # pred = torch.tensor(target)
                # pred[2:5, 7:13, 9:15] = 1 - pred[2:5, 7:13, 9:15]
                jaccard = JaccardIndex(task='multiclass', num_classes=len(label2cmap))
                iou = jaccard(outputs, labels)
                print("After jaccard")
                # Take the average to get the mean IoU
